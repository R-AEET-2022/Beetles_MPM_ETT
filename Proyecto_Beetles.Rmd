...
Mixed model with random = plot
```{r,,echo=FALSE, warning=FALSE,message=FALSE}
mix_mod_abundance = lmer(abundance ~ year +(1|plot), data = bet_sum)
summary(mix_mod_abundance)
...
```{r,echo=FALSE, warning=FALSE,message=FALSE}
plot(allEffects(mix_mod_abundance))
```

```{r,echo=FALSE, warning=FALSE,message=FALSE}
performance::check_model(mix_mod_abundance)
